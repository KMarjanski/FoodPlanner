...
const Koszyk = () => {
  ...
  const { selectedPlan, planners, ingredients, setIngredients, cart, setCart } =
  ...
  }, [koszykRaw, setKoszyk, cookRaw]);

  useEffect(() => {
    !!cart.length && setKoszykRaw(cart[0].cart);
  }, [cart, setKoszykRaw]);

  const filteredRecipes = Object.entries(
    planners?.find((p) => p.label === selectedPlan.selected) || {}
  )
    .filter((p) => p[0] !== "id" && p[0] !== "label" && p[0] !== "_id")
    .reduce((acc, curr) => {
  ...
  const ingredientsListRaw = filteredRecipes
    .map((fr) => ingredients.filter((i) => i.inRecipes.some((ir) => ir === fr)))
    .reduce((acc, curr) => {
      curr.forEach((c) => {
    ...
    .sort(sortByCategory);
  const missingIngredientsRaw = ingredients.filter(
    (i) => !koszykRaw.some((ilr) => ilr.name === i.name)
  );
  const missingIngredients = cookRaw(missingIngredientsRaw);
  const missingIngredientsWSearch = missingIngredients
    .reduce((acc, curr) => {
      acc.push({
        ...curr,
        ingredients: curr.ingredients.filter((i) =>
    ...
    .sort(sortByCategory);
  const ingredientsWSearch = cookRaw(ingredients)
    .reduce((acc, curr) => {
      acc.push({
        ...curr,
        ingredients: curr.ingredients.filter((i) =>
    ...
    .sort(sortByCategory);
  const isNotNewIngr = missingIngredientsWSearch.some(
    (miws) => !!miws.ingredients.length
  );
  const addToKoszykRaw = (name, category) =>
    setKoszykRaw((old) => [...old, { inRecipes: [], name, category }]);
        ...
        <center>
          <Row>
            ...
            <Col xs="auto" className="mx-0 px-0">
              <Button
                ...
                title={`Last saved: ${
                  cart[0] &&
                  cart[0]?.date &&
                  cart[0]?.date.replace("T", " ").replace("Z", "")
                }`}
              >
                {spinner ? (
              ...
              <Button
                onClick={() => {
                  fetch(`/toEvernote`, {
                    ...
                    body: JSON.stringify(
                      customSortCart(koszyk).map(
                        (k) =>
                          `<h3>${
                            k.category
                          }</h3><ul style="--en-todo:true;">${k.ingredients.map(
                            (i, j) =>
                              `<li${
                                j === 0 ? "" : ' style="--en-checked:false;"'
                              }><div>${i}</div></li>`
                          )}</ul>`
                      )
                    )
                  ...
                  navigator.clipboard.writeText(
                    JSON.stringify(customSortCart(koszyk))
                      .replaceAll("category", "")
            ...
            <Col xs="auto" className="me-2">
              <Button
                onClick={() => setKoszykRaw(ingredientsListRaw)}
                variant="success"
                size="lg"
          ...
          <Container fluid>
            {koszyk.sort(sortByCategory).map((il, i) => (
              <Container fluid key={i}>
                ...
                {il.ingredients.map((ing, j) => (
                  <Badge
                    ...
                    key={j}
                    onClick={() =>
                      setKoszykRaw((old) => old.filter((o) => o.name !== ing))
                    }
                    bg="success"
          ...
          <Container className="mb-4" fluid>
            {(isNotNewIngr
              ...
              <Container fluid key={i}>
                ...
                {il.ingredients.map((ing, j) => {
                  const isInCart = koszyk.some((k) =>
                    k.ingredients.some((i) => i === ing)
                  );
                  return (
                    <Badge
                      onClick={() =>
                      ...
                      <ButtonGroup vertical>
                        <Button
                          ...
                          onClick={() => {
                            ...
                            addToKoszykRaw(search, "Chemia");
                          }}
                        >
                          {getIngredientCategoryWithIcon("Chemia")}
                        </Button>
                        <Button
                          variant="success"
                          size="sm"
                          onClick={() => {
                            ...
                            addToKoszykRaw(search, "Dania gotowe");
                          }}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            addToKoszykRaw(search, "Mrożonki");
                          }}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            addToKoszykRaw(search, "Pieczywo");
                          }}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            addToKoszykRaw(search, "Przyprawy");
                          }}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            addToKoszykRaw(search, "Słodycze");
                          }}
                        >
